    ...
    const std::optional<turn_selection> &selection = std::nullopt)
{
    ...
    try
    {
        ...
        std::size_t actions = 0;
        if(selection && selection->actions == 0)
            ...
            node->variations_or_outcome))
        {
            ...
            ++actions;
            if(selection && actions == selection->actions)
                return parsed_position{std::move(current), actions};
            node = next.get();
        }
...
int run_print(int argc, const char *argv[])
{
    ...
    return with_position([](const parsed_position &p) { std::cout << p.value->to_string(); });
}
...
int run_diff(int argc, const char *argv[])
{
    ...
    return with_position([](const parsed_position &position) { diff(*position.value); });
}
