...
hc_move_evaluation::hc_move_evaluation(
    const HC_info& info,
    bool evaluate_checks)
    : info(info)
{
    if(evaluate_checks)
        checks.emplace(check_position::for_move_scoring(info.s));
}
    ...
    index_t coordinate)
{
    ...
    const bool branching = !physical
        && s.get_timeline_end(q.l()) != turn_t{q.t(), player};
    ...
    if(physical && (source->lpawn() & pmask(p.xy())) && p.x() != q.x()
       && captured == NO_PIECE)
    {
        captured = source->get_piece(ppos(q.x(), p.y()));
    }
    if(physical && (source->king() & pmask(p.xy()))
       && std::abs(p.x() - q.x()) > 1)
    {
    ...
    if(checks)
    {
        std::array<check_position::scoring_board, 2> updates{{
            {destination, next_turn({q.t(), player}), {q.t() + 1, player},
             cached->result},
            {p.l(), next_turn({p.t(), player}), {p.t() + 1, player},
             cached->departure}
        }};
        checking = checks->gives_check(
            std::span(updates).first(physical ? 1 : 2), player);
    }
    // Only the features consumed by weighting are constructed. No successor
    // state or detailed check classification is needed for this boolean.
    return extract_move_info_features({
        nullptr,
        vec4(q.x(), q.y(), q.t() + 1, destination),
        moved,
        captured,
        special,
        checking ? check_type_t::PHYSICAL_CHECK : check_type_t::NONE});
}
