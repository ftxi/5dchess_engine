...
+ An `ext_move` object (defined in `core/action.h`) stores a `full_move` plus which piece it promotes to. `NO_PIECE` means the choice is unspecified and should be resolved using the state's promotion rules. Canonical non-promotion moves store `NO_PIECE`.
